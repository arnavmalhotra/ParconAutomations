...
Global $firstSuccessfulData = False  ; New variable to track first successful data
...
Func WaitForPleaseWaitToDisappear()
    ...
    Local $startTime = TimerInit()
    Local $timeout = 600000  ; 15 seconds in milliseconds
    ...
    While TimerDiff($startTime) < $timeout
        ...
        If $handle = 0 Or Not StringInStr(ControlGetText("", "", $handle), "Please Wait") Then
            ...
            Return  ; "Please Wait" not found, exit the function and continue the script
        Else
            Sleep(1000)  ; "Please Wait" found, sleep for 1 second
        EndIf
    WEnd
    RestartProcess()
    WriteLog("WaitForPleaseWaitToDisappear: Timeout reached after 15 seconds.")
EndFunc
...
Func HandleSecurityDialog()
    ...
    Local $hWnd = WinWait("Windows Security", "", 30)
    If $hWnd Then
        WinActivate($hWnd)
        Send("!o")  ; Send Alt+O
        Sleep(1000)
...
Func HandleDeliveryInstructionsPopup()
    ...
    Local $timeout = 300000  ; 10 seconds timeout
...
Func EnterUserPin()
    ...
    WinWait("Verify User PIN", "", 10)  ; Wait up to 10 seconds for the window to appear
    ...
    If WinExists("Verify User PIN") Then
        ...
        WinWaitActive("Verify User PIN", "", 5)  ; Wait up to 5 seconds for the window to become active
        ...
        WinWaitClose("Verify User PIN", "", 10)  ; Wait up to 10 seconds for the window to close
...
Func ContinuousProcessing()
    ...
    Local $inactivityTimeout = 10 * 60 * 1000  ; 5 minutes in milliseconds

    While 1
        If CheckForDisconnection() Then
            WriteLog("Disconnection detected. Exiting ContinuousProcessing.")
            ExitLoop  ; Exit the loop instead of calling RestartProcess directly
        EndIf
        ...
        If TimerDiff($lastActivityTime) > $inactivityTimeout Then
            WriteLog("Bot inactive for more than 5 minutes. Exiting ContinuousProcessing.")
            ExitLoop  ; Exit the loop instead of calling RestartProcess directly
        EndIf
    ...
    ; After exiting the loop, call RestartProcess
    WriteLog("ContinuousProcessing exited. Calling RestartProcess.")
    RestartProcess()
EndFunc
...
Func ProcessDealBook($currentSaleNumber)
    WriteLog("Processing deal book for sale number: " & $currentSaleNumber)
    Local $dealBookReady = False
    While Not $dealBookReady
        $dealBookReady = CheckDealBook()
        If Not $dealBookReady Then
            WinActivate("e-Auction")
            OpenDO()
            Sleep(1000)
        EndIf
    ...
    For $i = 0 To UBound($aBidders) - 1
        ...
        WinActivate("e-Auction")
        ClickRefresh()
        WaitForPleaseWaitToDisappear()
        ; Check for "No data found" popup
        If DetectAndHandlePopup() Then 
            WriteLog("No data found for bidder: " & $aBidders[$i])
            ContinueLoop  ; Move to the next bidder if popup appears
        EndIf
        sleep(10000)
        ; Execute these steps for every successful data retrieval
        WinActivate("e-Auction")
        ...
        WaitForPleaseWaitToDisappear()
        ; If it's the first time data is found successfully
        If Not $firstSuccessfulData Then
            If $sCertificateSelection = "Auto" Then
            ...
            Sleep(2000)
            EnterUserPin()
            $firstSuccessfulData = True  ; Set flag to true after handling
        EndIf
        WinActivate("e-Auction")
        If CheckForDisconnection() Then
            RestartProcess()
